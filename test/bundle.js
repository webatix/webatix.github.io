 ...
 * subject to an additional IP rights grant found at
 * http://polymer.github.io/PATENTS.txt
 */;const E=new class{handleAttributeExpressions(e,t,o,n){const r=t[0];if("."===r){return new O(e,t.slice(1),o).parts}if("@"===r)return[new C(e,t.slice(1),n.eventContext)];if("?"===r)return[new k(e,t.slice(1),o)];return new _(e,t,o).parts}handleTextExpression(e){return new w(e)}};
/**
 * @license
 * Copyright (c) 2017 The Polymer Project Authors. All rights reserved.
 * This code may only be used under the BSD style license found at
...
"undefined"!=typeof window&&(window.litHtmlVersions||(window.litHtmlVersions=[])).push("1.3.0");const B=(e,...t)=>new v(e,t,"html",E);var T={object_entries_two_columns_view:e=>B`
      <div>
        <kor-table columns="120px 1fr auto">
      ...
      <kor-tab-item id="summaryTab" label="Summary" active
        @active-changed="${t=>{if(t.target.active){for(;e.lastChild;)e.removeChild(e.lastChild);template_node=e.appendChild(document.getElementById("summaryPageTemplate").content.cloneNode(!0))}}}">
        <template id="summaryPageTemplate"></template>
      </kor-tab-item>
      <kor-tab-item id="transactionsTab" label="Transactions"
        @active-changed="${t=>{if(t.target.active){for(;e.lastChild;)e.removeChild(e.lastChild);template_node=e.appendChild(document.getElementById("transactionsPageTemplate").content.cloneNode(!0))}}}">
        <template id="transactionsPageTemplate"></template>
      </kor-tab-item>
      <kor-tab-item id="scheduleTab" label="Schedule"
        @active-changed="${t=>{if(t.target.active){for(;e.lastChild;)e.removeChild(e.lastChild);template_node=e.appendChild(document.getElementById("schedulePageTemplate").content.cloneNode(!0))}}}">
        <template id="schedulePageTemplate"></template>
      </kor-tab-item>`};z(T.tab_strip_view(app_page_content),tab_strip);const A=document.getElementById("summaryPageTemplate").content;document.getElementById("transactionsPageTemplate").content,document.getElementById("schedulePageTemplate").content;const D=document.getElementById("summaryTab");document.getElementById("transactionsTab"),document.getElementById("scheduleTab"),app_page&&window.matchMedia&&window.matchMedia("(prefers-color-scheme: dark)").matches&&app_page.setAttribute("theme","dark"),app_bar&&viewport_width<viewport_height&&app_page.setAttribute("mobile","true"),app_settings.addEventListener("click",(()=>{app_settings_modal.setAttribute("visible","true")})),login_menu.addEventListener("click",(()=>{Moralis.Web3.authenticate().then((e=>{z(T.object_entries_two_columns_view(Object.entries(e)),A),D.setAttribute("active","true")})),app_settings_modal.removeAttribute("visible")})),document.addEventListener("DOMContentLoaded",(()=>{Moralis.initialize("2cf2XPXuDOixO5CVbNqtCfgFuJ6Bjf0hmLVwm6f1"),Moralis.serverURL="https://bkvqmakttdiy.moralis.io:2053/server"}));
