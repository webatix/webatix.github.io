...
"undefined"!=typeof window&&(window.litHtmlVersions||(window.litHtmlVersions=[])).push("1.3.0");const B=(e,...t)=>new v(e,t,"html",E);var T={tab_strip_view:(e,t,o)=>B`
      ...
      </kor-table>`,transactions_view:(e,t)=>B`
      <kor-grid spacing="1" columns="2">
        ...
        <kor-card grid-cols="1" grid-cols-m="2" grid-cols-s="2" flat>
          <kor-table columns="200px 1fr auto">
            ...
            ${e.map((e=>B`
                ${Object.entries(e.attributes).map((e=>B`
                    <kor-table-row>
                      <kor-table-cell>${e[0]}</kor-table-cell>
                      <kor-table-cell>${e[1]}</kor-table-cell>
                    </kor-table-row>`))}`))}
      ...
      </kor-grid>`},A={Init(e,t,o,n){const r=async e=>{const o=new Moralis.Query("EthTransactions");o.equalTo("to_address",e.get("ethAddress"));const n=new Moralis.Query("EthTransactions");n.equalTo("from_address",e.get("ethAddress"));let r=await o.find();console.log("incoming transactions:",r);let i=await n.find();console.log("out coming transactions:",i),z(T.transactions_view(r,i),t);const s=await o.subscribe(),c=await n.subscribe();s.on("create",(e=>{console.log("new incoming transactions:",data),r=r.concat(e),z(T.transactions_view(r,i),t)})),c.on("create",(e=>{console.log("new out coming transactions:",data),i=i.concat(e),z(T.transactions_view(r,i),t)}))};return()=>{if(!n.getAttribute("login"))return(async()=>{await Moralis.User.logOut(),z(B``,e),z(B``,t),z(B``,o),console.log("logged out")})(),n.setAttribute("label","Login using Metamask"),void n.setAttribute("login","true");Moralis.Web3.authenticate().then((t=>{n.setAttribute("label","Log out"),n.removeAttribute("login"),z(T.object_entries_two_columns_view(Object.entries(t)),e),console.log("current user:",t),console.log(t.get("ethAddress")),Moralis.Cloud.run("watchEthAddress",t.get("ethAddress")).then((e=>{console.log(e),r(t)})).catch((()=>{r(t)}))}))}}};const D=(()=>{const e={tmpl:document.getElementById("appTemplate").content},t=e.tmpl.querySelector("kor-page"),o=e.tmpl.getElementById("summaryContent"),n=e.tmpl.getElementById("transactionsContent"),r=e.tmpl.getElementById("scheduleContent"),i=e.tmpl.querySelector("kor-app-bar"),s=e.tmpl.getElementById("appSettings"),c=e.tmpl.getElementById("appSettingsModal"),a=e.tmpl.getElementById("loginMenuItem");e.tmpl.getElementById("prefsMenuItem");const l=e.tmpl.querySelector("kor-tabs"),h=Math.max(document.documentElement.clientWidth||0,window.innerWidth||0),d=Math.max(document.documentElement.clientHeight||0,window.innerHeight||0);z(T.tab_strip_view(o,n,r),l),e.tmpl.getElementById("summaryTab"),e.tmpl.getElementById("transactionsTab"),e.tmpl.getElementById("scheduleTab"),t&&window.matchMedia&&window.matchMedia("(prefers-color-scheme: dark)").matches&&t.setAttribute("theme","dark"),i&&h<d&&i.setAttribute("mobile","true"),s.addEventListener("click",(()=>{c.setAttribute("visible","true")})),a.setAttribute("login","true");const p=A.Init(o,n,r,a);return a.addEventListener("click",(()=>{c.removeAttribute("visible"),p()})),e})();document.addEventListener("DOMContentLoaded",(()=>{Moralis.initialize("pnj2ou0ivNkr2xnWIQIKZjwhcbJgC6hO69CasCTS"),Moralis.serverURL="https://kcmeib0ovv6c.moralis.io:2053/server";const e=document.getElementsByTagName("body")[0];e.removeChild(document.getElementById("startupContent")),e.appendChild(D.tmpl)}));
