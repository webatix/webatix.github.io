...
var K,X,Z,Q,G,Y;J.finalized=!0,J.shadowRootOptions={mode:"open"},null===(H=(L=globalThis).reactiveElementPlatformSupport)||void 0===H||H.call(L,{ReactiveElement:J}),(null!==(U=(F=globalThis).reactiveElementVersions)&&void 0!==U?U:F.reactiveElementVersions=[]).push("1.0.0-rc.1"),(null!==(K=(Y=globalThis).litElementVersions)&&void 0!==K?K:Y.litElementVersions=[]).push("3.0.0-rc.1");class tt extends J{constructor(){super(...arguments),this.renderOptions={host:this},this.Φt=void 0}createRenderRoot(){var t,e;const o=super.createRenderRoot();return null!==(t=(e=this.renderOptions).renderBefore)&&void 0!==t||(e.renderBefore=o.firstChild),o}update(t){const e=this.render();super.update(t),this.Φt=j(e,this.renderRoot,this.renderOptions)}connectedCallback(){var t;super.connectedCallback(),null===(t=this.Φt)||void 0===t||t.setConnected(!0)}disconnectedCallback(){var t;super.disconnectedCallback(),null===(t=this.Φt)||void 0===t||t.setConnected(!1)}render(){return w}}tt.finalized=!0,tt._$litElement$=!0,null===(Z=(X=globalThis).litElementHydrateSupport)||void 0===Z||Z.call(X,{LitElement:tt}),null===(G=(Q=globalThis).litElementPlatformSupport)||void 0===G||G.call(Q,{LitElement:tt});var et={tab_strip:(t,e,o)=>x`
      ...
      </kor-table>`,transactions:(t,e,o)=>x`
      <kor-grid columns="2">
      ...
      </kor-grid>`,add_transaction:(t,e,o,r)=>x`
      <kor-accordion label="Add a destination address">
        <kor-radio-button label="Choose from own addresses" active
          @active-changed="${t=>{const e=t.target.parentElement.querySelectorAll("kor-accordion")[0];t.target.active?e.style.display="block":e.style.display="none"}}">
        </kor-radio-button>
        <kor-accordion label="The list of own addresses" icon="view_list" style="display: block">
          ${e.map((t=>x`
              <kor-radio-button label="${t}"
                @active-changed="${e=>{e.target.active&&(r.destination_address=t)}}">
              </kor-radio-button>`))}
        </kor-accordion>
        <kor-radio-button label="Choose from booked addresses"
          @active-changed="${t=>{const e=t.target.parentElement.querySelectorAll("kor-accordion")[1];t.target.active?e.style.display="block":e.style.display="none"}}">
        </kor-radio-button>
        <kor-accordion label="The list of booked addresses" icon="view_list" style="display: none">
          ${o.map((t=>x`
              <kor-radio-button label="${t}"
                @active-changed="${e=>{e.target.active&&(r.destination_address=t)}}">
              </kor-radio-button>`))}
        </kor-accordion>
        <kor-radio-button label="Enter the destination address"
          @active-changed="${t=>{const e=t.target.parentElement.querySelector("kor-input");t.target.active?(e.style.display="block",r.destination_address=e.value):e.style.display="none"}}">
        </kor-radio-button>
        <kor-input label="The destination address:" type="text" value="" style="display: none"
      ...
      </kor-input>`,schedule:()=>x`

    `},ot={Init(t,e,o,r,i,n,s){const a=n.querySelector("div"),l=t=>{j(et.object_entries_two_columns(Object.entries(t.attributes)),a),n.setAttribute("visible","true")},c=async t=>{const o=new Moralis.Query("EthTransactions");o.equalTo("to_address",t.get("ethAddress"));const r=new Moralis.Query("EthTransactions");r.equalTo("from_address",t.get("ethAddress"));let i=await o.find();console.log("Incoming transactions: ",i);let n=await r.find();console.log("Out coming transactions: ",n),j(et.transactions(i,n,l),e);const a=await o.subscribe(),c=await r.subscribe();a.on("create",(t=>{console.log("A new incoming transaction: ",t),i=i.concat(t),j(et.transactions(i,n,l),e)})),c.on("create",(t=>{console.log("A new out coming transaction: ",t),n=n.concat(t),j(et.transactions(i,n,l),e)})),s.web3=await Moralis.Web3.enable(),console.log("web3 == ",s.web3)};return()=>{if(!r.getAttribute("login"))return Moralis.LiveQuery.close(),(async()=>{await Moralis.User.logOut(),j(x``,t),j(x``,e),j(x``,o),console.log("Logged out")})(),r.setAttribute("label","Login using Metamask"),r.setAttribute("login","true"),void(i.style.display="none");Moralis.Web3.authenticate().then((e=>{r.setAttribute("label","Log out"),r.removeAttribute("login"),i.style.display="block",j(et.object_entries_two_columns(Object.entries(e)),t),console.log("The current user: ",e),console.log("The current user password: ",e.password),console.log("The current user's EthAddress: ",e.get("ethAddress")),Moralis.Cloud.run("watchEthAddress",{address:e.get("ethAddress")}).then((t=>{console.log("The watchEthAddress results: ",t),c(e)})).catch((()=>{c(e)}))}))}}};const rt=(()=>{const t={tmpl:document.getElementById("appTemplate").content},e={content:t.tmpl.querySelector("kor-page"),top_bar:t.tmpl.querySelector("kor-app-bar"),menu:{button:t.tmpl.getElementById("appMenu"),tooltip:t.tmpl.getElementById("appMenuTooltip"),popover:{content:t.tmpl.getElementById("appMenuPopover"),login:t.tmpl.getElementById("loginMenuItem"),prefs:t.tmpl.getElementById("prefsMenuItem")}},add_transaction:t.tmpl.getElementById("addTransaction"),add_transaction_tooltip:t.tmpl.getElementById("addTransactionTooltip"),summary:{content:t.tmpl.getElementById("summaryContent"),details:t.tmpl.getElementById("summaryDetails")},transactions:{content:t.tmpl.getElementById("transactionsContent"),details:t.tmpl.getElementById("transactionsDetails"),filter_button:t.tmpl.getElementById("transactionsFilterButton")},schedule:{content:t.tmpl.getElementById("scheduleContent"),details:t.tmpl.getElementById("scheduleDetails"),add_button:t.tmpl.getElementById("scheduleAddButton")},tab_strip:t.tmpl.querySelector("kor-tabs")},o={window:t.tmpl.getElementById("addTransactionModal"),content:t.tmpl.getElementById("addTransactionModalContent"),button:t.tmpl.getElementById("addTransactionModalButton")},r=t.tmpl.getElementById("transactionDetailsModal"),i={window:t.tmpl.getElementById("addScheduleModal"),content:t.tmpl.getElementById("addScheduleModalContent"),button:t.tmpl.getElementById("addScheduleModalButton")};j(et.tab_strip(e.summary,e.transactions,e.schedule),e.tab_strip),e.content&&window.matchMedia&&window.matchMedia("(prefers-color-scheme: dark)").matches&&(e.content.setAttribute("theme","dark"),o.window.setAttribute("theme","dark"),r.setAttribute("theme","dark"),i.window.setAttribute("theme","dark")),e.menu.button.addEventListener("click",(()=>{e.menu.tooltip.setAttribute("visible","false")}));let n={};e.menu.popover.login.setAttribute("login","true");const s=ot.Init(e.summary.details,e.transactions.details,e.schedule.details,e.menu.popover.login,e.add_transaction,r,n);e.menu.popover.login.addEventListener("click",(()=>{e.menu.popover.content.removeAttribute("visible"),s()}));let a={gas_price:1,gas_limit:21e3};return e.add_transaction.addEventListener("click",(()=>{e.add_transaction_tooltip.setAttribute("visible","false"),j(et.add_transaction(6,[],[],a),o.content),o.window.setAttribute("visible","true")})),o.button.addEventListener("click",(()=>{if(o.window.removeAttribute("visible"),console.log("The transaction's destination_address == ",a.destination_address),console.log("The transaction's amount == ",a.amount),a.destination_address&&a.amount>0){const t={from:Moralis.User.current().get("ethAddress"),to:a.destination_address,value:a.amount*Math.pow(10,18),gasPrice:a.gas_price,gas:a.gas_limit};console.log("The transactionObject == ",t),n.web3.eth.sendTransaction(t,((t,e)=>{console.log("The sendTransaction error == ",t),console.log("The sendTransaction result == ",e),a.destination_address="",a.amount=0}))}})),e.schedule.add_button.addEventListener("click",(()=>{j(et.add_transaction(6,[],[],a),i.content),i.window.setAttribute("visible","true")})),t})();document.addEventListener("DOMContentLoaded",(()=>{Moralis.initialize("pnj2ou0ivNkr2xnWIQIKZjwhcbJgC6hO69CasCTS"),Moralis.serverURL="https://kcmeib0ovv6c.moralis.io:2053/server";const t=document.getElementsByTagName("body")[0];t.removeChild(document.getElementById("startupContent")),t.appendChild(rt.tmpl)}));
