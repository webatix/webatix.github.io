 ...
 * subject to an additional IP rights grant found at
 * http://polymer.github.io/PATENTS.txt
 */;const E=new class{handleAttributeExpressions(e,t,o,n){const r=t[0];if("."===r){return new j(e,t.slice(1),o).parts}if("@"===r)return[new C(e,t.slice(1),n.eventContext)];if("?"===r)return[new k(e,t.slice(1),o)];return new _(e,t,o).parts}handleTextExpression(e){return new w(e)}};
/**
 * @license
 * Copyright (c) 2017 The Polymer Project Authors. All rights reserved.
 * This code may only be used under the BSD style license found at
 ...
 * http://polymer.github.io/PATENTS.txt
 */
"undefined"!=typeof window&&(window.litHtmlVersions||(window.litHtmlVersions=[])).push("1.3.0");const B=(e,...t)=>new v(e,t,"html",E);var T={object_entries_two_columns_view:e=>B`
      <div>
        <kor-table columns="150px 1fr auto">
      ...
      </div>`,tab_strip_view:e=>B`
      <kor-tab-item id="summaryTab" label="Summary" active
        @active-changed="${t=>{if(t.target.active){for(;e.lastChild;)e.removeChild(e.lastChild);e.appendChild(document.getElementById("summaryPageTemplate").content.cloneNode(!0))}}}">
        <template id="summaryPageTemplate"></template>
      </kor-tab-item>
      <kor-tab-item id="transactionsTab" label="Transactions"
        @active-changed="${t=>{if(t.target.active){for(;e.lastChild;)e.removeChild(e.lastChild);e.appendChild(document.getElementById("transactionsPageTemplate").content.cloneNode(!0))}}}">
        <template id="transactionsPageTemplate"></template>
      </kor-tab-item>
      <kor-tab-item id="scheduleTab" label="Schedule"
        @active-changed="${t=>{if(t.target.active){for(;e.lastChild;)e.removeChild(e.lastChild);e.appendChild(document.getElementById("schedulePageTemplate").content.cloneNode(!0))}}}">
        <template id="schedulePageTemplate"></template>
      </kor-tab-item>`};const A=(()=>{const e={tmpl:document.getElementById("appTemplate").content},t=e.tmpl.querySelector("kor-page"),o=e.tmpl.querySelector("kor-grid"),n=e.tmpl.querySelector("kor-app-bar"),r=e.tmpl.getElementById("appSettings"),i=e.tmpl.getElementById("appSettingsModal"),s=e.tmpl.getElementById("loginMenuItem");e.tmpl.getElementById("prefsMenuItem");const c=e.tmpl.querySelector("kor-tabs"),a=Math.max(document.documentElement.clientWidth||0,window.innerWidth||0),l=Math.max(document.documentElement.clientHeight||0,window.innerHeight||0);z(T.tab_strip_view(o),c);const h=e.tmpl.getElementById("summaryPageTemplate").content,d=e.tmpl.getElementById("transactionsPageTemplate").content,p=e.tmpl.getElementById("schedulePageTemplate").content,b=e.tmpl.getElementById("summaryTab");e.tmpl.getElementById("transactionsTab"),e.tmpl.getElementById("scheduleTab"),t&&window.matchMedia&&window.matchMedia("(prefers-color-scheme: dark)").matches&&t.setAttribute("theme","dark"),n&&a<l&&t.setAttribute("mobile","true"),r.addEventListener("click",(()=>{i.setAttribute("visible","true")}));const f=()=>{b.active&&b.toggleAttribute("active"),b.toggleAttribute("active")};return s.addEventListener("click",(()=>{if(i.removeAttribute("visible"),"Log out"==s.label)return(async()=>{await Moralis.User.logOut(),z(B``,h),z(B``,d),z(B``,p),f(),console.log("logged out")})(),void s.setAttribute("label","Login using Metamask");Moralis.Web3.authenticate().then((e=>{s.setAttribute("label","Log out"),z(T.object_entries_two_columns_view(Object.entries(e)),h),console.log("current user:",e),(async e=>{const t=new Moralis.Query("EthTransactions");t.equalTo("from_address",e.get("ethAddress"));const o=await t.find();console.log("user transactions:",o);let n={};o.map((e=>{n={...n,...e.attributes}})),z(T.object_entries_two_columns_view(Object.entries(n)),d),(await t.subscribe()).on("create",(function(e){z(T.object_entries_two_columns_view(Object.entries(e)),d),console.log("user transactions from the subscription:",e)}))})(e),f()}))})),e})();document.addEventListener("DOMContentLoaded",(()=>{Moralis.initialize("2cf2XPXuDOixO5CVbNqtCfgFuJ6Bjf0hmLVwm6f1"),Moralis.serverURL="https://bkvqmakttdiy.moralis.io:2053/server";let e=document.getElementsByTagName("body")[0];e.removeChild(document.getElementById("startupContent")),e.appendChild(A.tmpl)}));
